...
def main():
    ...
    target_joint_positions = [
        -0.6283, -1.0647, 2.7227,  # FL
        -0.6283, -1.0647, 2.7227,  # FR  
        -0.6283, 1.0647, -2.7053,  # RL
        -0.6283, 1.0647, -2.7053   # RR
    ]
    ...
    try:
        ...
        for i in range(5):
            # 获取关节状态（已经是转换后的关节位置）
            joint_state = new_controller.get_joint_states()
            joint_positions = joint_state['positions']  # 这已经是关节位置，不是电机位置
            
            # 将模型顺序转换为基础顺序进行验证
            base_joint_positions = new_controller.model_to_base_order(joint_positions)
            joint_positions_samples.append(base_joint_positions)
            print(f"验证采样 {i+1}/5: {[f'{pos:.4f}' for pos in base_joint_positions]}")
            time.sleep(0.1)
