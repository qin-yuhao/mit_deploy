...
class StandUpConfig(PoseConfig):
    ...
    def __post_init__(self):
        if self.pose is None:
            self.pose = [
                -0.07, -0.6, 1.2,    # FL_hip_joint, FL_thigh_joint, FL_calf_joint
                -0.1, -0.85, 1.2,    # FR_hip_joint, FR_thigh_joint, FR_calf_joint
                -0.0,  0.6, -1.2,   # RL_hip_joint, RL_thigh_joint, RL_calf_joint
                -0.0,  0.6, -1.2    # RR_hip_joint, RR_thigh_joint, RR_calf_joint
            ]
        if self.kp is None:
            self.kp = [28] * 12
        if self.kd is None:
